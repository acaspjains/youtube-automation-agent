...
class Database {
  ...
  async saveContentStrategy(strategy) {
    const result = await this.executeQuery(
      `INSERT INTO content_strategies (id, topic, angle, target_audience, content_type, keywords, estimated_views, best_publish_time, created_at) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))`,
      [
        'strategy_' + Date.now() + '_' + Math.random().toString(36).slice(2,8),
        strategy.topic || 'Daily life story',
        strategy.angle || strategy.topic || 'Real story',
        strategy.targetAudience || process.env.TARGET_AUDIENCE || 'Middle class Indians aged 20-40',
        strategy.contentType || 'story',
        JSON.stringify(strategy.keywords || []),
        strategy.estimatedViews || 5000,
        strategy.bestPublishTime || new Date().toISOString()
      ]
    );
  ...
  async saveTrend(t) { try { await this.executeQuery('INSERT OR IGNORE INTO settings (key,value) VALUES (?,?)',["trend_"+Date.now(),JSON.stringify(t)]); } catch(e) {} }
  async saveSEO(s) { return this.saveSEOData(s); }
  async saveLearning(vid,metric,val,insight) { try { await this.executeQuery('INSERT OR IGNORE INTO settings (key,value) VALUES (?,?)',["learning_"+Date.now(),JSON.stringify({vid,metric,val,insight})]); } catch(e) {} }
  async getBestStrategies() { try { return await this.getAllRows('SELECT * FROM content_strategies ORDER BY created_at DESC LIMIT 10'); } catch(e) { return []; } }
  async saveVideo(v) { try { const r=await this.executeQuery('INSERT INTO productions (script_id,status,output_format,file_size,duration) VALUES (?,?,?,?,?)',[v.scriptId||null,v.status||'ready','mp4',0,0]); return r.lastID; } catch(e) { return null; } }
  async updateVideoYouTubeId(vid,ytId) { try { await this.setSetting('last_youtube_id_'+vid, ytId); } catch(e) {} }

  async saveTrend(t) { try { await this.executeQuery("INSERT OR IGNORE INTO settings (key,value) VALUES (?,?)",["trend_"+t.topic.slice(0,20),JSON.stringify(t)]); } catch(e) {} }
  async saveSEO(s) { return this.saveSEOData(s); }
  async saveLearning(vid,metric,val,insight) { try { await this.executeQuery("INSERT OR IGNORE INTO settings (key,value) VALUES (?,?)",["learning_"+Date.now(),JSON.stringify({vid,metric,val,insight})]); } catch(e) {} }
  async getBestStrategies() { try { return await this.getAllRows("SELECT * FROM content_strategies ORDER BY created_at DESC LIMIT 10"); } catch(e) { return []; } }
  async saveVideo(v) { try { const r=await this.executeQuery("INSERT INTO productions (script_id,status,output_format,file_size,duration) VALUES (?,?,?,?,?)",[v.scriptId||null,v.status||"ready","mp4",0,0]); return r.lastID; } catch(e) { return null; } }
  async updateVideoYouTubeId(vid,ytId) { try { await this.setSetting("last_youtube_id",ytId); } catch(e) {} }
